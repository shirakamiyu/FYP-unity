...
public class OpenManeger : MonoBehaviour
{
    public Image _image;
    public Image _hkiit;
    public Image _fyp;
    ...
    {
        // 
        yield return _hkiit.DOFade(1, 1).WaitForCompletion(); // 渐入效果
        yield return _hkiit.DOFade(0, 1).WaitForCompletion(); // 渐出效果

        //
        yield return _fyp.DOFade(1, 1).WaitForCompletion(); // 渐入效果
        yield return _fyp.DOFade(0, 1).WaitForCompletion(); // 渐出效果

        //
        yield return _image.DOFade(1, 1).WaitForCompletion(); // 渐入效果
        yield return _image.DOFade(0, 1).WaitForCompletion(); // 渐出效果
